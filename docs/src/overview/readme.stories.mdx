...
Find more about [Icon component](?path=/docs/components-icon--page).

### For Expo user

#### 1. In somwhere like `App.tsx`, load dooboo-ui font.

```js
import {useFonts} from 'expo-font';

const [fontsLoaded] = useFonts({
  doobooui: require('dooboo-ui/uis/Icon/doobooui.ttf'),
});
```

### For React Native user

#### 1. Create `react-native.config.cjs` file and add below code.
...
```
> Above will prevent adding default font icons from [react-native-vector-icons](https://github.com/oblador/react-native-vector-icons) in `ios` platform.

#### 2. Add rnpm config in `package.json`.

```
"rnpm": { "assets": [ "dooboo-ui/uis/Icon" ] },
```

#### 3. Run `yarn react-native link`.
